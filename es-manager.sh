    ...
    case "$1" in
        ...
        demo)
            ...
            if [ -z "$2" ]; then
                ...
                echo "  6) Wikipedia Full-Text Search"
                echo "  7) Property Relationships via Denormalized Index"
                echo "  8) Natural Language Examples"
                echo "  9) 🏡 Rich Real Estate Listing (Default)"
                echo
                echo -e "  ${CYAN}=== Hybrid & Location-Aware Demos ===${NC}"
                echo "  10) Hybrid Search with RRF"
                echo "  11) Location Understanding (DSPy)"
                echo "  12) 🌊 Location-Aware: Waterfront Luxury"
                echo "  13) 🏫 Location-Aware: Family Schools"
                echo "  14) 🏔️ Location-Aware: Recreation Mountain"
                echo "  15) 🎯 Location-Aware Search Showcase (Multiple)"
                echo "  16) Wikipedia Location Search"
                echo
                echo "Usage: $0 demo [1-16] [--verbose]"
                echo "Running default demo 9..."
                echo
            fi
            run_demo "${2:-9}" "$3"
            ;;
        hybrid-demo)
            ...
            if [ -z "$2" ]; then
                ...
                echo "  16) Wikipedia Location Search"
                echo
                echo "Usage: $0 hybrid-demo [10-16] [--verbose]"
                echo "Running default hybrid demo 10..."
                echo
            fi
            run_demo "${2:-10}" "$3"
            ;;
    ...
    while true; do
        ...
        case $choice in
            ...
            8)
                ...
                echo "  9) 🏡 Rich Real Estate Listing (Default)"
                echo
                read -p "Enter demo number (1-9, default: 9): " demo
                run_demo "${demo:-9}"
                ;;
            9)
                echo
                ...
                echo "  16) Wikipedia Location Search"
                echo
                read -p "Enter demo number (10-16, default: 10): " demo
                run_demo "${demo:-10}"
                ;;
            0)
                print_info "Exiting..."
