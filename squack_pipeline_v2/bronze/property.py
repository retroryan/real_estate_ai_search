...
class PropertyBronzeIngester(BronzeIngester):
    ...
    def ingest(
        ...
        """Ingest raw property data from JSON file(s).
        
        Bronze principle: NO transformations, just raw load.
        Now supports loading multiple property files.
        ...
        if file_path is not None:
            ...
            files_to_load = [Path(f) for f in self.settings.data_sources.properties_files]
            self.logger.info(f"Loading {len(files_to_load)} property files from settings")
        
        # Validate all files exist
        for file in files_to_load:
            if not file.exists():
                raise FileNotFoundError(f"Properties JSON file not found: {file}")
        ...
        # Load each file
        for file in files_to_load:
            self.logger.info(f"Loading raw properties from: {file}")
        ...
        self.logger.info(f"Total: Loaded {self.records_ingested} raw property records into {table_name}")
