...
class QueryGenerator:
    ...
    def _generate_geographic_queries(self, n: int) -> List[tuple]:
        ...
        if norcal_articles:
            ...
            queries.append((
                "Northern California regions and cities",
                [a["page_id"] for a in norcal_articles[:3]],
                relevance
            ))
        ...
        if south_utah:
            relevance = self._calculate_keyword_relevance("southern", summary_weight=2)
            queries.append((
                "Southern Utah locations and landmarks",
                [a["page_id"] for a in south_utah[:3]],
                relevance
            ))
        ...
        if bay_area:
            ...
            queries.append((
                ...
                [a["page_id"] for a in bay_area[:3]],
                relevance
            ))
        
        # Query 8: Mountain regions
        mountain = self.articles_by_category.get("mountain", [])
        if mountain:
